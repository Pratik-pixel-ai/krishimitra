    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-green-900">Crop Guide</h1>
            <p className="text-gray-500 text-sm">Season-wise info, MSP prices & farming tips</p>
          </div>
        </div>
          ...
          <div className="mt-4 bg-green-50 border border-green-200 rounded-2xl px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-green-800">
                Your soil: <strong>{farmSoil}</strong> — <span className="text-green-600">{suitableCount} suitable crops highlighted</span>
              </span>
            </div>
            <button
      ...
      {/* Filters */}
      <div className="bg-white rounded-3xl shadow-sm border border-green-100 p-5 mb-6">
        {/* Search */}
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-800 placeholder-gray-400 text-sm"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-y-4 gap-x-6">
              ...
              {seasons.map(s => (
                <button key={s} onClick={() => setSelectedSeason(s)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                    selectedSeason === s ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}>{s}</button>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {waterLevels.map(w => (
                <button key={w} onClick={() => setSelectedWater(w)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                    selectedWater === w ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}>{w}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 mb-2 flex items-center gap-1">
              <Leaf className="w-3 h-3" /> SOIL TYPE
            </p>
            <div className="flex flex-wrap gap-1.5">
              {soilTypes.map(s => (
                <button key={s} onClick={() => setSelectedSoil(s)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                    selectedSoil === s ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}>{s}</button>
              ))}
            </div>
          </div>
            ...
            >
              ...
              {suitable && (
                <div className="bg-green-600 text-white text-xs font-semibold text-center py-1.5 tracking-wide">
                  ✓ Suitable for your {farmSoil} soil
                </div>
              )}
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  <div className="bg-white/70 rounded-2xl p-2.5 text-center">
                    ...
                    <p className="text-xs text-gray-400 leading-none mb-0.5">Temp</p>
                    <p className="text-xs font-bold text-gray-700">{crop.temp}</p>
                  </div>
                  <div className="bg-white/70 rounded-2xl p-2.5 text-center">
                    <Droplets className={`w-3.5 h-3.5 mx-auto mb-1 ${waterColor[crop.water] || 'text-blue-500'}`} />
                    <p className="text-xs text-gray-400 leading-none mb-0.5">Water</p>
                    <p className={`text-xs font-bold ${waterColor[crop.water] || 'text-gray-700'}`}>{crop.water}</p>
                  </div>
                  <div className="bg-white/70 rounded-2xl p-2.5 text-center">
                    <Clock className="w-3.5 h-3.5 text-green-500 mx-auto mb-1" />
                    <p className="text-xs text-gray-400 leading-none mb-0.5">Duration</p>
                    <p className="text-xs font-bold text-gray-700">{crop.duration.split(' ')[0]}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-4">
                  <div className="bg-white/70 rounded-2xl px-3 py-2">
                    <div className="flex items-center gap-1 mb-0.5">
                      ...
                      <p className="text-xs text-gray-400 font-medium">MSP Price</p>
                    </div>
                    <p className="text-xs font-bold text-green-700 leading-snug">{crop.msp}</p>
                  </div>
                  <div className="bg-white/70 rounded-2xl px-3 py-2">
                    <div className="flex items-center gap-1 mb-0.5">
                      <TrendingUp className="w-3 h-3 text-blue-500" />
                      <p className="text-xs text-gray-400 font-medium">Avg Yield</p>
                    </div>
                    <p className="text-xs font-bold text-blue-700 leading-snug">{crop.yield}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {crop.soil.map((s, j) => (
                    <span key={j} className="text-xs bg-white/80 text-gray-600 border border-gray-200 px-2.5 py-0.5 rounded-full">
                      {s}
                    </span>
                  ))}
                </div>
                ...
                {/* Months */}
                <div className="flex items-center gap-2 mb-4 bg-white/60 rounded-xl px-3 py-2">
                  <Sun className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                  <p className="text-xs text-gray-600"><span className="font-semibold">Growing:</span> {crop.months} &nbsp;·&nbsp; {crop.duration}</p>
                </div>
                ...
                <button
                  ...
                  style={{ color: crop.accent, backgroundColor: 'rgba(255,255,255,0.6)' }}
                >
                  {isOpen ? <><ChevronUp className="w-3.5 h-3.5" /> Hide details</> : <><ChevronDown className="w-3.5 h-3.5" /> Full details</>}
                </button>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-200/60 space-y-4">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Best States</p>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        {crop.states.map((state, j) => (
                          <span key={j} className="text-xs bg-white text-gray-700 border border-gray-200 px-2.5 py-0.5 rounded-full shadow-sm">
                            {state}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Common Pests / Diseases</p>
                      </div>
                      <p className="text-sm text-gray-700 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2">{crop.pest}</p>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Farming Tips</p>
                      </div>
                      <p className="text-sm text-gray-700 leading-relaxed bg-white/80 rounded-xl px-3 py-2.5 border border-gray-100">{crop.tips}</p>
                    </div>

                    {/* Profit potential */}
                    <div className="flex items-center justify-between bg-white/80 rounded-xl px-3 py-2.5 border border-gray-100">
                      <div className="flex items-center gap-1.5">
                        <TrendingUp className="w-3.5 h-3.5 text-gray-400" />
                        <p className="text-xs font-semibold text-gray-500">Profit Potential</p>
                      </div>
                      <span className={`text-sm font-bold ${profitColor[crop.profit] || 'text-gray-600'}`}>
                        {crop.profit}
                      </span>
                    </div>
                  </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20">
          <Sprout className="w-14 h-14 mx-auto mb-4 text-gray-200" />
          <p className="text-lg font-semibold text-gray-500">No crops match your filters</p>
          <p className="text-sm text-gray-400 mt-1">Try clearing a filter or changing your search</p>
          <button
            onClick={() => { setSearch(''); setSelectedSeason('All'); setSelectedWater('All'); setSelectedSoil('All') }}
            className="mt-4 text-sm text-green-600 font-medium underline underline-offset-2"
          >
            Clear all filters
          </button>
        </div>
